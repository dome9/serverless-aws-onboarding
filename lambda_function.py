...
class LambdaHandler(object):
    MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME = 'Dome9AutomaticOnboardingStackSet'
    ...
    def create_stack_set(self) -> Dict:
        ...
        logger.info(f"Creating StackSet: {self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}, "
        ...
        response = self.cloudformation_client.create_stack_set(
            StackSetName=LambdaHandler.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME,
    ...
    def create_stack_instances(self) -> None:
        ...
        :return:
        """
        logger.info(f"Creating stack instance for StackSet: {self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}, "
        ...
        response = self.cloudformation_client.create_stack_instances(
            StackSetName=self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME,
        ...
        for retry_count in range(self.STACK_OPERATION_WAIT_RETRIES):
            ...
            try:
                response = self.cloudformation_client.describe_stack_set_operation(
                    StackSetName=self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME, OperationId=operation_id)
                current_status = response.get('StackSetOperation').get('Status')
            except Exception as e:
                logger.error(f"Failed to fetch operation's status {current_status} wither error: {repr(e)}")
            ...
            if current_status == "SUCCEEDED":
                ...
                Check 'cloudformation' StackSet '{self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}' for more information")

            logger.info(f"Going to sleep for {self.STACK_OPERATION_WAIT_SLEEP} seconds")

            time.sleep(self.STACK_OPERATION_WAIT_SLEEP)
        else:
            raise OperationFailedError(f"Timeout reached waiting for operation '{operation_name}' with id '{operation_id}'")
    ...
    def delete_stack_instances(self) -> None:
        ...
        logger.info(f"Deleting stack instance for StackSet: {self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}, "
                    f"region: {self.region_name}, AccountId: {self.customer_account_id}")
    ...
    def delete_stack_set(self) -> Dict:
        ...
        logger.info(f"Deleting StackSet: {self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}")
        return self.cloudformation_client.delete_stack_set(StackSetName=self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME)
    ...
    def create_stack_set_flow(self) -> None:
        ...
        try:
            ...
            if "NameAlreadyExistsException" in repr(e):
                logger.error(
                    f"Received NameAlreadyExistsException for stack_set '{self.MASTER_ACCOUNT_ONBOARDING_STACK_SET_NAME}'. Error: {repr(e)}")
